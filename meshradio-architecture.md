...
**The listening record.** `plays` had been written since v1 and never read except by the cache pruner. `/stats` now surfaces it: recently played (grouped by video, so a song replayed twice doesn't push the list off the page), most played, and completion — `plays.completed` distinguishes "played" from "sat through", which is a thing only this instance can know. It is deliberately a *listening* chart next to the existing *posting* chart (`top_songs`), and the two disagreeing is the interesting part. The chart starts at two plays: at one, it's just the recently-played list again. Timestamps are stored UTC and rendered channel-local, like every other date on the site.
