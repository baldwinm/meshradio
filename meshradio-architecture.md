...
The calendar answers "what happened on this day"; **`/archive/themes`** answers the other question members ask — "what have we done already?" It lists every theme the channel has run (`Database.all_themes`), newest first, grouped by month, each linking to its day. `Untitled — <date>` placeholders are filtered out: nobody chose them, and their days are still on the calendar. `context.theme_history` counts how many days share a title (matched case- and space-insensitively) so a repeat carries an `N×` badge — the cheapest way to see whether "rain songs" has been done before. The route is declared *before* `/archive/{date}` so the path isn't taken for a date.
